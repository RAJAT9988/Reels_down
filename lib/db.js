...
function assertDbMode() {
  if (useTurso()) return "turso";
  // In production (including Vercel), the filesystem is not guaranteed writable.
  // Only allow local sqlite fallback during development unless explicitly enabled.
  if (isVercel() || (isProd() && process.env.ALLOW_LOCAL_DB_IN_PROD !== "1")) {
    throw new Error(
      "Database is not configured on Vercel. Set TURSO_DATABASE_URL and TURSO_AUTH_TOKEN in your Vercel Project → Settings → Environment Variables, then redeploy."
    );
  }
  return "local";
}
